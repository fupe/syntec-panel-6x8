...
void keydown(kb_inst_t *inst){
    ...
    *inst->hal.key[r * inst->ncols + c] = 1;
    ...
    if (inst->scan){ //scanning request
            ...
            for (c = 0; c < inst->ncols; c++){
                ...
                if ((inst->then[inst->row] & mask) && !(scan & mask)){ //keyup
                    *inst->hal.keycode = inst->keyup 
                    + (inst->thenrow << inst->rowshift) // original + (inst->row << inst->rowshift)
                    + c;
                    keyup(inst);
                }
		...
		inst->thenrow = inst->nowrow;
		inst->nowrow = inst->row;
		
		*inst->hal.keynumber = 0;
		for (c = 0; c < inst->nrows; c++){
		nokey += inst->now[c] & ~(0xFFFFFFFF << inst->rowshift) ;
		//*inst->hal.scan = inst->keydown ;
		}
		if (nokey == 0) {
			*inst->hal.nokey = 1;
		}
        ...
        *inst->hal.rows[inst->row] = !inst->param.invert;
		...
		//--------------diody------------
		for (c = 0; c < inst->ncols; c++){
			*inst->hal.cols_led[c]= *inst->hal.led[inst->row*inst->ncols+c];
		}
		*inst->hal.led[*inst->hal.ledcode & 0xff] = (*inst->hal.ledcode >> 8) ;
...
int rtapi_app_main(void){
    int i, j, n;
    int retval;
    comp_id = hal_init("matrix_kb_led");
    if (comp_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb_led: ERROR: hal_init() failed\n");
        return -1;
    }
    ...
    if (kb == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "matrix_kb_led component: Out of Memory\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    if (n && n != kb->num_insts){
        rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb_led: Number of sizes and number"
                        " of names must match\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (i = 0; i < kb->num_insts; i++){
        ...
        if (inst->ncols == 0 || inst->nrows == 0){
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "matrix_kb_led: Invalid size format. should be NxN\n");
            hal_exit(comp_id);
            return -1;
        }
        ...
        if (inst->ncols > 32){
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "matrix_kb_led: maximum number of columns is 32. Sorry\n");
            hal_exit(comp_id);
            return -1;
        }
		...
		inst->hal.led = (hal_bit_t **)hal_malloc(inst->nrows * inst->ncols * sizeof(hal_bit_t*));
        ...
        {
            rtapi_snprintf(inst->name, sizeof(inst->name), "matrix_kb_led.%i", i);
        }
        ...
        for (c = 0; c < inst->ncols; c++){
            for (r = 0; r < inst->nrows; r++){  
                ...
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "matrix_kb_led: Failed to create output pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
				retval = hal_pin_bit_newf(HAL_OUT,
                                          &(inst->hal.led[r * inst->ncols + c]), 
                                          comp_id,
                                          "%s.led.r%xc%x", 
                                          inst->name, r, c);
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "matrix_kb_led: Failed to create input led pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }
        }
		inst->hal.cols_led = (hal_bit_t **)hal_malloc(inst->ncols * sizeof(hal_bit_t*));
		for (c = 0; c < inst->ncols; c++){
                retval = hal_pin_bit_newf(HAL_OUT,
                                          &(inst->hal.cols_led[c]), comp_id,
                                          "%s.col-led%02i-out",inst->name, c);
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "matrix_kb_led: Failed to create output col-led pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
        ...
        if (inst->scan){ //internally generated scanning
            ...
            for (r = 0; r < inst->nrows; r++){
                ...
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "matrix_kb_led: Failed to create output row pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }
            for (c = 0; c < inst->ncols; c++){
                ...
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR,
                                    "matrix_kb_led: Failed to create input col pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }
			...
			retval = hal_pin_bit_newf(HAL_OUT,&(inst->hal.nokey), comp_id,"%s.nokey",inst->name);
			...
			retval = hal_pin_u32_newf(HAL_OUT,
                                      &(inst->hal.keynumber), comp_id,
                                      "%s.keynumber",inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR,
                                "matrix_kb_led: Failed to create output pin\n");
                hal_exit(comp_id);
			...
			retval = hal_pin_u32_newf(HAL_IN,
                                      &(inst->hal.ledcode), comp_id,
                                      "%s.ledcode",inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR,
                                "matrix_kb_led: Failed to create input pin\n");
                hal_exit(comp_id);
                return -1;
            }
			
			retval = hal_pin_u32_newf(HAL_OUT,
                                      &(inst->hal.scan), comp_id,
                                      "%s.scan",inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR,
                                "matrix_kb_led: Failed to create output scan pin\n");
                hal_exit(comp_id);
                return -1;
            }
            ...
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR,
                                "matrix_kb_led: Failed to create rollover param\n");
                hal_exit(comp_id);
                return -1;
            }
        ...
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb_led: ERROR: function export failed\n");
            return -1;
        }
    }
    hal_ready(comp_id);
